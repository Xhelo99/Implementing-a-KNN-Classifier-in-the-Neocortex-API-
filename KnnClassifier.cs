...
namespace KNN
{
    public class KnnClassifier<TInput, TOutput> : IClassifierKnn<string, ComputeCycle>
    {
        ...
        // Constructor to initialize training data dictionary
        ...
        // Method to calculate distance between predictive cells and training samples
        private double CalculateDistance(Cell[] predictiveCells, List<Cell[]> trainingSamples)
        {
            double sum = 0;

            // Calculate distance for each training sample
            foreach (var sample in trainingSamples)
            {
                double squaredSum = 0;

                // Calculate squared Euclidean distance for each dimension (cell index)
                int minLength = Math.Min(predictiveCells.Length, sample.Length);
                for (int i = 0; i < minLength; i++)
                {
                    double diff = predictiveCells[i].Index - sample[i].Index;
                    squaredSum += diff * diff;
                }

                // Add square root of squared distance to sum
                sum += Math.Sqrt(squaredSum);
            }

            // Return average distance
            return sum / trainingSamples.Count;
        }
